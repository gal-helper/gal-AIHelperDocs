...
COMMENT ON TABLE ai_conversation_info IS '会话信息记录表';
COMMENT ON COLUMN ai_conversation_info."id" IS 'ID';
COMMENT ON COLUMN ai_conversation_info."user_intent" IS '用户意图';
COMMENT ON COLUMN ai_conversation_info."conversation_memory" IS '会话记忆';
COMMENT ON COLUMN ai_conversation_info."create_time" IS '创建时间';
COMMENT ON COLUMN ai_conversation_info."update_time" IS '修改时间';

-- 创建消息记录表
CREATE TABLE ai_message_info (
    "id"                 INT8 PRIMARY KEY,                      -- ID
    "fk_conversation_id" INT8 NOT NULL,                         -- 会话id (外键，关联会话表)
    "fk_message_id"      INT8,                                  -- 对应的消息id，AI回应用户
    "sender_type"        INT4,                                  -- 发送者类型 (如: 1-用户, 2-AI)
    "receiver_type"      INT4,                                  -- 接收者类型
    "message"            TEXT,                                  -- 消息内容
...
COMMENT ON COLUMN ai_message_info."fk_conversation_id" IS '会话id';
COMMENT ON COLUMN ai_message_info."fk_message_id" IS '对应的消息id';
COMMENT ON COLUMN ai_message_info."sender_type" IS '发送者类型';
COMMENT ON COLUMN ai_message_info."receiver_type" IS '接收者类型';
COMMENT ON COLUMN ai_message_info."message" IS '消息内容';
COMMENT ON COLUMN ai_message_info."send_time" IS '发送时间';
...
-- 7. 初始数据录入
INSERT INTO sys_sequence (seq_name, current_value) VALUES ('ai_conversation_info', 1000);
