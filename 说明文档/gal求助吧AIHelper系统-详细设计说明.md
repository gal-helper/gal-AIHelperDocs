...
| ------------------------------------------------- | ------------------------------------------------------ |
| 垂直领域专业性强；AI 交互体验优于传统论坛。       | 知识库初期整理工作量大；API 调用成本。                 |
...
| **后端框架**       | **Spring Boot 3.x**          | Java 生态旗舰，开发快，资料多，对 LangChain4j 支持完美。     |
| **AI 框架**        | **LangChain4j**              | Java 界最好的 LangChain 实现，完美适配 Spring Boot 声明式编程。 |
| **数据库**         | **PostgreSQL 16 + pgvector** | 避免引入额外的向量数据库（如 Milvus），降低维护成本，支持 SQL 与向量混合查询。 |
| **AI 模型 (待定)** | **DeepSeek / GPT-4o-mini**   | 建议初期使用 DeepSeek-V3（国产模型，理解 Galgame 语境极佳且价格便宜）。 |
| **知识库检索**     | **Embedding (未定)**         | RAG的知识检索所需的用户对话文本向量化进行匹配                |
